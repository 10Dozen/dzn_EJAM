...
if (_case == "case_not_ejected") then {
	if (_mag == "mag_attached" && _hasAmmo) then {
		["bolt_not_closed","chamber_stucked",nil,nil] call FUNC(setWeaponState);
	} else {
		["bolt_not_closed",nil,nil,nil] call FUNC(setWeaponState);
	};
} else {
	if (_chamber == "chamber_stucked") then {
		...
		if (_chamber == "chamber_empty") then {
			if (_mag == "mag_attached" && _hasAmmo) then {
				["bolt_closed","chamber_ready",nil,nil] call FUNC(setWeaponState);
			} else {
				["bolt_closed",nil,nil,nil] call FUNC(setWeaponState);
			};
		} else {
			if (_chamber in ["chamber_not_extracted","chamber_ready"]) then {
				if (_mag == "mag_attached" && _hasAmmo) then {
					...
					if (_chamber == "chamber_ready") then { REMOVE_ROUND; };
					["bolt_closed","chamber_empty",nil,nil] call FUNC(setWeaponState);
				};
			};
		};
	};
